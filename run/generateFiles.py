...
modelName = 'own'
modelFolderPath = f'/home/slava/speech_commands/sphinx/{modelName}'
...
with open(transcriptionFilePath, 'w') as transcriptionFile:
    ...
    for phone in util.consonant_phones:
        for word in util.getConsonantWords(phone):
            key = '_'.join(word)
            transcriptionFile.write("<s> " + " ".join(word) + f' </s> ({fileIds[key]})\n')
...
try:
    subprocess.run(
        ...
        shell=True, stdout=subprocess.DEVNULL, stderr=subprocess.STDOUT).check_returncode()
